...
export default function Dashboard({ people, circles, user, onReload, onSignOut }) {
  ...
  const reloadAfter = async (operation) => {
    setActionError("");
    try { await operation(); await onReload(); clearSelection(); return true; }
  ...
  const handleMore = async (person, action) => {
    setMoreFor(null);
    const ids = resolveTargets(person);
    if (action === "dismiss") return reloadAfter(() => updatePeople(ids, { suggestion_dismissed_at: new Date().toISOString() }));
    if (action === "block") return setRelationshipDialog({ ids, kind: "block", suggestionIds });
    if (action === "favorite") return reloadAfter(() => setFavorites(ids, !person.favorite));
  };
  const confirmRelationship = async () => {
    ...
    const success = kind === "unfollow"
      ...
      : kind === "block"
        ? await reloadAfter(() => blockPeople(ids))
  ...
  const handleAction = async (person, action) => {
    ...
    const ids = resolveTargets(person);
    return reloadAfter(() => updateFollowing(ids, true));
  };
  ...
  const isInteractionLocked = isDialogOpen;
  ...
  </div>{sortOpen && !isInteractionLocked && <div className="sort-scrim" aria-hidden="true" />}{isDialogOpen && <div className="modal-scrim" aria-hidden="true" />}
